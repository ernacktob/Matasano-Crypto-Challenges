...
static int sha1_mac(const uint8_t *message, size_t len, uint32_t mac[5])
{
	...
	if (sha1_hash(modified_message, 8 * (keylen + len), mac) != 0) {
...
static int check_authentication(const uint8_t *message, size_t len, const uint32_t mac[5])
{
	uint32_t calc_mac[5];
	int i;
...
int check_admin(const uint8_t *message, size_t len, const uint32_t mac[5])
{
...
const uint8_t *get_url(size_t *urllen, uint32_t mac[5])
{
	const char *url = "comment1=cooking%20MCs;userdata=foo;comment2=%20like%20a%20pound%20of%20bacon";

	*urllen = strlen(url);

	if (sha1_mac(url, *urllen, mac) != 0) {
		perror("sha1_mac failed");
		return NULL;
	}
...
uint8_t *get_length_extended_url(size_t *newlen, uint32_t new_mac[5], const uint8_t *url, size_t urllen, const uint32_t mac[5], const uint8_t *append, size_t appendlen)
{
	...
	for (klen = 0; klen < 20; klen++) {
		...
		*newlen = urllen + padlen + appendlen;
		...
		if (sha1_update(append, 8 * appendlen, mac, 8 * (klen + urllen + padlen), new_mac) != 0) {
			free(padding);
			free(new_url);
			return NULL;
		}
