...
{
	uint32_t h0 = prev_hh[4];
	uint32_t h1 = prev_hh[3];
	uint32_t h2 = prev_hh[2];
	uint32_t h3 = prev_hh[1];
	uint32_t h4 = prev_hh[0];
	...
	memcpy(msg, message, ml / 8);
	msg[ml / 8] = 0x80;
	*((uint64_t *)(msg + (ml + 8 + rem) / 8)) = to_big_endian64(pl + ml);
